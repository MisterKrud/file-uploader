...
const cloudinaryUpload = async (req, res, next) => {
  ...
  try {
   if(mimeType.match(/audio/)|| mimeType.match(/video/)){
...
const cloudinaryDeleteFolderKeepFiles = async (req, res, next) => {
  ...
  const files = await db.getAllFilesInFolder(parentFolder.id, req.user.id);
  const fileAssetIds = [];
...
const getAllFoldersAndFiles = async (req, res, next) => {
  if (req.user) {
    ...
    if (req.params.folderId) {
      parentFolder = await db.getParentFolder(
...
const createFolder = async (req, res) => {
  ...
  const newFolder = await db.createUserFolder(
    req.user.id,
    req.body.folderName,
    parentFolder.id
  );
